...
//			}
			...
			for(StartEvent se : model.getStartEvents()) {
				...
				seTag.addAttribute("eventRef", se.getId());
				...
				seTag.addAttribute("id", se.getId() + "_gui");
				xPosition += 28 + 50;
			}
			for(Task t : model.getTasks()) {
				SXTag tTag = laneCompartment.addChildNode("bpmndi:activityShape");
				tTag.addAttribute("activityRef", t.getId());
				...
				tTag.addAttribute("name", t.getName());
				tTag.addAttribute("id", t.getId() + "_gui");
				xPosition += 100 + 50;
			}
			for(EndEvent ee : model.getEndEvents()) {
				SXTag eeTag = laneCompartment.addChildNode("bpmndi:eventShape");
				eeTag.addAttribute("eventRef", ee.getId());
				...
				eeTag.addAttribute("id", ee.getId() + "_gui");
				xPosition += 28 + 50;
			}
			
			for(Sequence s : model.getSequences()) {
				SXTag sTag = processDiagram.addChildNode("bpmndi:sequenceFlowConnector");
				sTag.addAttribute("sequenceFlowRef", s.getId());
				sTag.addAttribute("label", "");
				sTag.addAttribute("sourceRef", s.getSource().getId() + "_gui");
				sTag.addAttribute("targetRef", s.getSink().getId() + "_gui");
				sTag.addAttribute("id", s.getId() + "_gui");
			}
